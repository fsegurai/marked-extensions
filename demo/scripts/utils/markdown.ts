...
// Side-effect import: emits the dedicated extensions stylesheet (see extension-styles.ts).
...
export const mdRender = (md: string, mdBody: HTMLElement | null) => {
  ...
  // Opt-in: make Kanban cards draggable between columns (native HTML5 DnD).
  // On the playground page, the move is synced back to the editable source via
  // the window hook; elsewhere (read-only index) the hook is absent → no-op.
  enableKanbanDragDrop(mdBody, {
    onMove: (detail) => window.updateMarkdownFromKanbanMove?.(detail),
  });
};
...
export const updateCheckboxInMarkdown = (markdown: string, rawText: string | undefined, checked: boolean): string => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (lineTaskMatch) {
      ...
      if (lineTaskText.trim() === taskText && lineIndentDepth === rawIndentDepth && lineChecked === wasChecked) {
