import type { KanbanMoveDetail, TaskListMetadata } from '../../packages/bundle/src/index';
import { applyMoveToMarkdown, enableSourceListEditing } from '../../packages/bundle/src/index';
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Wire source-level list editing (Enter to continue/break, Tab indent/outdent)
  const textarea = editor.shadowRoot?.querySelector('textarea');
  if (textarea) {
    enableSourceListEditing(textarea);
  }
...
window.updateMarkdownFromKanbanMove = (detail: KanbanMoveDetail) => {
  if (!editor) return;

  const currentMarkdown = editor.value;
  const updatedMarkdown = applyMoveToMarkdown(currentMarkdown, detail);
