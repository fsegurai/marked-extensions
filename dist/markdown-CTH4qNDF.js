...
class _Tokenizer {
    ...
    table(src) {
        ...
        const headers = splitCells$1(cap[1]);
        ...
        for (const row of rows) {
            item.rows.push(splitCells$1(row, item.header.length).map((cell, i) => {
                return {
                    text: cell,
...
// Define prefixes and suffixes for different element types
const elementPatterns = {
  accordionBlock: {
    start: '::::accordion',
    end: '::::accordionend',
    aliases: [':acc', ':accordion'],
    endAliases: [':accend', ':accordionend'],
  },
  spoilerBlock: {
    start: '::::spoiler',
    end: '::::spoilerend',
    aliases: [':sp', ':spoiler'],
    endAliases: [':spend', ':spoilerend'],
  },
  tabBlock: {
    start: '::::tabs',
    end: '::::tabsend',
    aliases: [':tb', ':tabs'],
    endAliases: [':tbend', ':tabsend'],
  },
  tabItemBlock: {
    start: ':::tab',
    end: ':::tabend',
    aliases: [':t', ':tab'],
    endAliases: [':tend', ':tabend'],
  },
  timelineBlock: {
    start: '::::timeline',
    end: '::::timelineend',
    aliases: [':tl', ':timeline'],
    endAliases: [':tlend', ':timelineend'],
  },
  timelineEventBlock: {
    start: ':::event',
    end: ':::eventend',
    aliases: [':ev', ':event'],
    endAliases: [':evend', ':eventend'],
  },
};
...
const tabItemPatterns = generateRegexFromPattern('tabItemBlock');
...
const timelineBlockRegex = new RegExp(
  `^${timelinePatterns.startPattern}\\s*(?:\\n|$)((?:[\\s\\S]*?)(?:${timelineEventPatterns.startPattern}\\{.*?\\}[\\s\\S]*?${timelineEventPatterns.endPattern}(?:\\n|$))*)${timelinePatterns.endPattern}\\s*(?:\\n|$)`,
);
const timelineEventRegex = new RegExp(
  `${timelineEventPatterns.startPattern}\\{(.*?)\\}([\\s\\S]*?)${timelineEventPatterns.endPattern}`,
  'g',
);

// Define supported properties for each element type
...
/**
 * Generate regex patterns from element patterns configuration
 * @param {string} elementType - Element type to generate regex for
 * @returns {Object} Object with generated regex patterns
 */
...
function parseBalancedTags(src, elementType) {
  ...
  // Track which specific patterns were matched (for start and end)
  let startPos = 0;

  if (src.startsWith(start)) {
    startPos = start.length;
  } else {
    // Check for aliases (shortcuts)
    ...
    // No matching start pattern found
  ...
  // Parse content with balanced tag awareness
  ...
  while (pos < src.length && depth > 0) {
    ...
    // Then check all aliases for the earliest occurrence
    for (const alias of aliases) {
    ...
    // Malformed if no closing tag found
    if (nextEndPos === -1) return null;

    // Handle nested start tag
    if (nextStartPos !== -1 && nextStartPos < nextEndPos) {
      depth++;
      // Determine which specific pattern was matched
    ...
    // Found matching end tag at current depth
    if (--depth === 0) {
      content = src.substring(contentStart, nextEndPos);
      const fullMatch = src.substring(0, nextEndPos + foundEndPattern.length);

      // Create RegExp-compatible result object
...
const validateRegex = (element, src) => {
  ...
  switch (element) {
    ...
    case 'tabBlock':
      return tabBlockRegex.exec(src);
    case 'tabItemBlock':
      return tabItemRegex.exec(src);
    case 'timelineBlock':
      return timelineBlockRegex.exec(src);
    case 'timelineEventBlock':
      return timelineEventRegex.exec(src);
    default:
 ...
 *
 * @param {string} element - The type of element to construct properties for
 * @param {string} propString - String containing property attributes in format 'name="value"'
 * @returns {Object} An object containing all supported properties with their parsed values
 */
const constructProps = (element, propString) => {
  ...
  const propPattern = propRegex;
  let propMatch;
  while ((propMatch = propPattern.exec(propString)) !== null) {
 ...
 * @param {Object} options - Properties for the accordion
 * @param {String} options.prefixId - Prefix for the accordion ID
...
function createTokenizer$4(options = {}) {
  ...
  return {
    ...
    tokenizer(src) {
      ...
      const blockMatch = validateRegex(element, src);
...
function markedExtendedAccordion(options = {}) {
  ...
  return {
    walkTokens(token) {
      if (token.type !== 'accordion') return;
 ...
 * @param {Object} options - Configuration options
 * @param {string} [options.placeholderText='Footnotes'] - Placeholder text for footnote section
 * @param {Function} [options.escapeRegExp] - Function to escape regex characters
 * @param {Object} [options.state] - Shared state object for footnotes
 * @returns {Object} Tokenizer functions
 */
    ...
    footnoteRef: {
      ...
      tokenizer(src) {
        ...
        if (match) {
          ...
          const number = state.footnoteRefs.get(id).number;
...
function createFootnoteRenderers(options = {}) {
  const { prefixId, labelFormat, refMarkers } = options;

  return {
    footnoteRefRenderer(token) {
      const id = `${prefixId}${token.id}`;
      const refId = `${prefixId}ref-${token.id}`;
      const displayLabel = labelFormat(token.id, token.number);

      return `<sup><a id="${refId}" href="#${id}" data-fnref-ref aria-describedby="${options.prefixId}label">${
        refMarkers ? `[${displayLabel}]` : displayLabel
      }</a></sup>`;
    },
 ...
 * @param {Object} options - Configuration options
 * @param {string} [options.prefixId='footnote-'] - Prefix for footnote IDs
 * @param {string} [options.description='Footnotes'] - Description for accessibility
 * @param {boolean} [options.backrefLinks=true] - Whether to include backlinks
 * @param {string} [options.backrefSymbol='↩'] - Symbol used for backlinks
 * @param {Function} [options.labelFormat=(id, number) => number] - Function to format the footnote label
 * @param {Object} state - Shared state object for footnotes
      ...
      for (const [id, def] of state.footnoteDefinitions) {
        if (state.footnoteRefs.has(id)) {
          validFootnotes.push({
            id,
            content: def.content,
            number: state.footnoteRefs.get(id).number,
          });
        }
      }
 ...
 * @param {Object} options - Configuration options
 * @param {string} [options.prefixId='fnref-'] - Prefix for footnote IDs
 * @param {string} [options.description='Footnotes'] - Description for accessibility
 * @param {boolean} [options.refMarkers=false] - Whether to show reference markers in brackets
 * @param {string} [options.backrefSymbol='↩'] - Symbol used for backlinks
 * @param {boolean} [options.backrefLinks=true] - Whether to include backlinks
 * @param {string} [options.placeholderText='[footnotes]'] - Custom placeholder text to specify where footnotes appear
 * @param {Function} [options.labelFormat=(id, number) => number] - Function to format the footnote label
 * @returns {Object} Marked extension object
 */
function markedExtendedFootnote(options = {}) {
  ...
  const state = {
    ...
    isProcessingFootnoteSection: false, // This flag prevents recursion when generating the footnote section
    resetFootnotes() {
  ...
  // Create tokenizers and renderers
  ...
  const renderers = createFootnoteRenderers({ ...config });
    ...
    extensions: [
      {
        ...
        renderer: renderers.footnoteRefRenderer,
      },
      {
        ...
        renderer: renderers.footnoteDefRenderer,
      },
      {
        ...
        renderer: () => generateFootnoteSection(),
      },
    ],
    hooks: {
      postprocess(html) {
        // Only add footnotes if needed, not already added, and not currently processing
        if (
          state.footnoteRefs.size > 0
          ...
          // Clear refs since we've processed them
 ...
 * @param {string} letter - The letter to convert (e.g., 'a', 'b', 'c')
 * @returns {number} The integer value (e.g., 'a' -> 1, 'z' -> 26)
 */
...
/**
 * Renders a checkbox for task lists
 * @param {Object} options - Checkbox options
 * @returns {string} HTML for the checkbox
 */
function renderCheckbox(options) {
  return `<input ${options.checked ? 'checked="" ' : ''}disabled="" type="checkbox">`;
}

/**
 * Creates a tokenizer function for parsing lists in Markdown
 * @returns {Function} The list tokenizing function
 */
function createTokenizer$3() {
 ...
 * @returns {Object} The list and listitem renderer functions
 */
function createRenderer$1() {
  return {
    /**
     * Renderer for list tokens
     * @param {Object} token - The list token to render
     * @returns {string} HTML for the list
     */
    list(token) {
      const ordered = token.ordered;
      const listType = token.listType || '1';
      let body = '';
    ...
    /**
     * Renderer for list item tokens
     * @param {Object} item - The list item token to render
    ...
    listitem(item) {
      ...
      if (item.task) {
        // Define checkbox directly since the checkbox renderer might not be available
        const checkbox = `<input ${item.checked ? 'checked="" ' : ''}disabled="" type="checkbox">`;
  
        if (item.loose) {
          // Handle loose task items
      ...
      // Add value attribute for skipped numbers
 ...
 */
function markedExtendedLists() {
  return {
    tokenizer: {
      list: createTokenizer$3(),
    },
    renderer: createRenderer$1(),
    hooks: {
      // Add a checkbox renderer function to the Marked instance
      postprocess(html) {
        return html;
      },
    },
    // Add the checkbox renderer function to the extensions
    extensions: [
      {
        name: 'checkbox',
...
const SPOILER_THEMES = {
  default: {
    overlayBg: 'rgba(60, 60, 60, 1)',
    textColor: 'white',
    shadow: '0 0 12px rgba(60, 60, 60, 0.3)',
  },
  dark: {
  ...
  light: {
    overlayBg: 'rgba(240, 240, 240, 1)',
    textColor: '#333',
    shadow: '0 0 12px rgba(200, 200, 200, 0.6)',
  },
  danger: {
    overlayBg: 'rgba(220, 53, 69, 1)',
    textColor: 'white',
    shadow: '0 0 12px rgba(220, 53, 69, 0.4)',
  },
  info: {
    overlayBg: 'rgba(13, 110, 253, 1)',
    textColor: 'white',
    shadow: '0 0 12px rgba(13, 110, 253, 0.4)',
  },
  warning: {
    overlayBg: 'rgba(198, 144, 38, 1)',
    textColor: 'white',
    shadow: '0 0 12px rgba(198, 144, 38, 0.4)',
  },
  success: {
    overlayBg: 'rgba(87, 171, 90, 1)',
    textColor: 'white',
    shadow: '0 0 12px rgba(87, 171, 90, 0.4)',
  },
};
 ...
 * @param {string} theme - The theme of the spoiler for color adaptation.
 * @returns {Object} Object containing particlesContent and particleStyles
 */
...
/**
 * Render the spoiler content with hover effect and theme-based shadows.
 * @param {Object} options - The options for rendering the spoiler.
...
function renderSpoiler(options = {}) {
  ...
  const customTitle = title ? `<span class="spoiler-title">${title}</span>` : '<span class="spoiler-title"></span>';
  ...
  // Generate enhanced hover shadow based on the theme shadow
  const themeShadow = themeStyles.shadow || '0 2px 8px rgba(0, 0, 0, 0.15)';
  const themeShadowHover = themeShadow
    .replace(/(\d+)px/g, (match) => `${parseInt(match) * 1.5}px`)
    .replace(
      /rgba\((\d+),\s*(\d+),\s*(\d+),\s*([\d.]+)\)/g,
      (_, r, g, b, a) => `rgba(${r}, ${g}, ${b}, ${Math.min(parseFloat(a) * 1.2, 0.9)})`,
    );
...
function markedExtendedSpoiler(options = {}) {
  ...
  return {
    ...
    extensions: [createSpoilerEffect(config)],
  };
}
 ...
 * @param {Object} cell - Cell object with rowspan and colspan properties
 * @param {string} type - The type of cell ('th' or 'td')
 * @param {string|null} align - Alignment of the cell ('left', 'right', 'center', or null)
 ...
 *
 * @param {string} tableRow - The raw table row text
 * @param {number|null} count - Number of columns to normalize to (null for no normalization)
 * @param {Array|null} prevRow - Previous row for handling row spans
 * @param {number|null} maxColspan - Maximum number of columns a cell can span (null for no limit)
 * @returns {Array} Array of processed table cells
 */
 ...
 *
 * @param {Array} cells - Raw cells to process
 * @param {number|null} count - Number of columns to normalize to
 * @param {Array} prevRow - Previous row for handling row spans
 * @param {number|null} maxColspan - Maximum number of columns a cell can span
 * @returns {Array} Processed table cells
 */
const processSpans = (cells, count, prevRow = [], maxColspan = null) => {
        ...
        if (
          ...
          if (cell.colspan > 1 && prevCell.colspan > 1) {
            ...
            if (cell.colspan === prevCell.colspan && cell.position === prevCell.position) {
              ...
              cell.rowSpanTarget.text += ` ${cellText.slice(0, -1)}`;
  ...
  return normalizeColumnCount(cells, count, numCols);
};

/**
 * Ensures the row has the correct number of columns
 *
 * @param {Array} cells - The cells to normalize
 * @param {number|null} count - The target column count (null for no normalization)
 * @param {number} numCols - Current number of columns
 * @returns {Array} Normalized cells
 */
 ...
 * @param {number} [options.maxColspan=null] - Maximum number of columns a cell can span (null for no limit)
...
function createTokenizer$2(options = {}) {
  ...
  return function tokenizer(src) {
    ...
    if (colCount !== item.align.length) return null;
...
// The rest of your existing functions with modifications to use options
...
function processRows(item, colCount, lexer, maxColspan) {
  ...
  // Add inline tokens to header cells
  ...
  // Add inline tokens to body cells
...
function createRenderer(options = {}) {
  ...
  return function renderer(token) {
    ...
    if (token.caption) {
      const captionHtml = `<caption>${this.parser.parseInline(token.captionTokens || [])}</caption>\n`;
    ...
    if (token.rows.length || token.footerRow) {
      ...
      if (useTheadTbody) {
        ...
        for (const row of token.rows) {
          output += renderRow(row, token.align, this.parser, 'td', handleComplexSpans);
        }
        output += '</tbody>\n';
      } else {
        for (const row of token.rows) {
      ...
      if (token.footerRow && useTfoot) {
        output += '<tfoot>\n';
        output += renderRow(token.footerRow, token.align, this.parser, 'td', handleComplexSpans);
        output += '</tfoot>\n';
      } else if (token.footerRow) {
        // If there's a footer row from detectFooter but we're not using tfoot
        output += renderRow(token.footerRow, token.align, this.parser, 'td', handleComplexSpans);
      } else if (useTfoot && token.rows.length > 0) {
        // If useTfoot is true but no footer was detected, use the last row
        const lastRow = token.rows.pop();
        output += '<tfoot>\n';
        output += renderRow(lastRow, token.align, this.parser, 'td', handleComplexSpans);
        output += '</tfoot>\n';
      }
    }

    // Add caption at bottom if specified
    if (token.caption && !captionTop)
      output += `<caption>${this.parser.parseInline(token.captionTokens || [])}</caption>\n`;
 ...
 * @param {boolean} [options.detectFooter=false] - Whether to detect and mark the last row as a footer
 * @param {boolean} [options.strictCaptions=false] - Whether to enforce strict caption syntax
 * @param {number} [options.maxColspan=null] - Maximum number of columns a cell can span
 * @param {boolean} [options.handleComplexSpans=true] - Better handling of complex row+col spans
 * @returns {Object} Marked extension object
 */
 ...
 * @param {Function|null} formatter - Optional custom formatter function
  ...
  return {
    ...
    tokenizer(src) {
      ...
      while ((eventMatch = timelineEventRegex.exec(timelineContent)) !== null) {
        ...
        const eventContent = eventMatch[2].trim();

        // Parse properties with defaults
        const props = constructProps(timelineEventElement, propsStr);

        // If no date is provided, use a default
        if (!props.date) props.date = `Event ${eventIndex}`;

        // Format the date
        props.formattedDate = formatDate(props.date, dateFormat);
...
const SmartProcess = (options = {}) => {
  ...
  // Generate cache key based on text content and configuration
...
const SmartPants = (options = {}) => {
  const {
    ...
    attribute = '2',
    customSymbols = {},
    outputUnicode = false, // New parameter to control output format
  } = options;
...
/**
 * Transforms hyphens into proper en-dashes and em-dashes while preserving HTML structure
 *
 * @param {Object} options - Configuration options
 * @param {string} [options.html=''] - HTML text to process
 * @param {string|number} [options.attribute='2'] - Typography style attribute
 * @return {string} Processed HTML with smart dashes
 */
const SmartDashes = (options = {}) => {
  ...
  if (attr === '0') return html; // Do nothing
 ...
 * @param {string} [options.html=''] - HTML text to process
 * @param {string|number} [options.attribute='2'] - Typography style attribute
 * @return {string} Processed HTML with smart ellipses
 */
    ...
    hooks: {
      postprocess(html) {
        try {
          ...
          if (config.quotes) result = SmartQuotes({ html: result, ...config });

          if (config.dashes) result = SmartDashes({ html: result, ...config });

          if (config.ellipses) result = SmartEllipses({ html: result, ...config });
